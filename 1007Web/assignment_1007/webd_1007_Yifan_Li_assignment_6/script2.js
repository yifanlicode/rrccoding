...
function createHTML(Data) {
    ...
    //only show the breeds "shiba"
    const breedsObject = Data.message;
    const breedsArray = Object.keys(breedsObject);//这是一个数组 代表所有的breeds
    const subBreed = Object.values(breedsObject);  //这是一个数组 代表了每个breed的subBreed的数组

    for (let i = 0; i <  breedsArray.length; i++) {
        if(breedsArray[i] == "hound"){  
